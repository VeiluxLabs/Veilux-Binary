...
| **Staking & Governance Prism** ✅ | bond native LUX, delegate, stake-weighted on-chain proposals & voting, **slashing**, **reward pool** | Shipped: escrowed stake, delegation, proposal lifecycle, equivocation slashing (auto-detected from consensus votes), and a proportional reward pool with `fund_rewards`/`claim_rewards` (`prisms/staking`). |
| **Fee market / gas economics** ✅ | per-command fees charged at execution, split burn + proposer reward, **dynamic base fee** | Shipped: deterministic `fee = gas × base_price`, configurable burn fraction, capped at payer balance, EIP-1559-style base-fee that tracks `fee_target_gas`; configured at genesis (node-level). |
